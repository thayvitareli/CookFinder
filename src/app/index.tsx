...
import { View , Text, StyleSheet, ListRenderItemInfo, Image, FlatList, TouchableOpacity, ActivityIndicator} from "react-native";
...
export default function Home(){
    ...
    const getAllCategories = async()  => {
        ...
        return response.meals.map((meal : {strCategory: string}) => meal.strCategory);
    }
    ...
    const getAllMeals = async () => {
        ...
        let allMeals : iMeal[] = responses.flatMap(response => response.data.meals);
    ...
    const RenderItem = ({item}: ListRenderItemInfo<iMeal>) => {
        return<TouchableOpacity style={styles.card} onPress={() => handlePress(item.idMeal)}>
